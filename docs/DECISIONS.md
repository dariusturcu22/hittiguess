...
## 2026-09 | Story 9 built (backend): DJ link-out endpoint, no server-side "opened the link" state

Decision: story 9's backend slice is a single DJ-gated read endpoint, `GET /api/sessions/{sessionId}/link-out`, returning the current round's YouTube video id and canonical watch URL through `RoundLinkOutDTO`. Access is restricted to the round's own DJ (a non-DJ player and a non-member are both denied) and refused once the round is `REVEALED` or `SCORED`, since by then the song is already public and the DJ's playback window is over. The watch URL is built by a new `YoutubeLinkParser.buildWatchUrl` from the stored video id rather than assembled from a hardcoded string inline. This exists because `RoundDTO` deliberately withholds a song's `youtubeId` until the reveal, so a DJ client otherwise has no way to reach the video to link out. Round flow stays driven by the active player's placement-lock-in timers, not by the DJ opening the link, so "the DJ opened the link" carries no server-side state and is a story-28 client concern. The guess-lock-in audio-stream cutoff is likewise a story-28 client reaction to the existing `GUESS_LOCKED` broadcast, not new backend state. The DJ view, the "Open YouTube Link" action and its audio-sharing warning, WebRTC tab-audio capture, and deep-link handling are all deferred to story 28 under the standing all-frontend-in-story-28 policy.

Why: the one real backend gap for story 9 was that playback data never reached the DJ. Everything else the story lists is frontend or WebRTC, which lives in story 28. Gating the link-out to the DJ and only before the reveal keeps the endpoint from becoming a way for any player to read an unrevealed song's identity, which would defeat the round. Building the watch URL through a named helper keeps the compliance-critical "real YouTube page, never an embedded player" rule in one place rather than scattered as string literals. Adding a server-side "DJ opened the link" flag was rejected because nothing in round flow reads it: the timers already fire off placement lock-in, so such a flag would be dead state.
...
## 2026-09 | Story 38 observability: everything short of the account-dependent pieces built now, degrading safely to no-op without credentials

Decision: story 38 is split at the boundary of what needs a real third-party account and what doesn't, and everything on the no-account side is built now rather than deferred alongside the account-dependent pieces. The core service uses `spring-boot-starter-actuator` and `micrometer-registry-prometheus`, exposing `/actuator/health` and `/actuator/prometheus`. The AI microservice uses `prometheus-fastapi-instrumentator` for a `/metrics` endpoint and a plain `/health` route. Both services get OpenTelemetry instrumentation: the core service via `opentelemetry-spring-boot-starter` (`io.opentelemetry.instrumentation:opentelemetry-instrumentation-bom` pinned at 2.30.0), the AI microservice via `opentelemetry-instrumentation-fastapi` with a manually installed `TracerProvider`. Both default to producing real in-process spans without exporting anywhere: the core service through `otel.sdk.disabled`, defaulting to `true` until `OTEL_SDK_DISABLED=false` is set; the AI microservice by only attaching an OTLP exporter when `OTEL_EXPORTER_OTLP_ENDPOINT` is actually set. Both services get Sentry SDK wiring (`sentry-spring-boot-4`, `sentry-sdk`), inactive by the SDK's own default behavior whenever no DSN is configured, so `sentry_sdk.capture_exception` and its Java equivalent are safe no-ops until a real DSN exists. Structured logging in both services renders each log line as one JSON object (timestamp, level, logger, message, request id, trace/span id) to stdout, a shape a Loki/Promtail pipeline can scrape without either service depending on a live Loki endpoint to run or be tested; the core service uses a small hand-written Logback encoder rather than a third-party JSON encoder library, avoiding a dependency whose Jackson-version compatibility with this project's already-bleeding-edge Spring Boot 4 stack couldn't be confirmed without a real build. A `CorrelationIdFilter` (core service) and `CorrelationIdMiddleware` (AI microservice) assign or reuse an `X-Request-Id` header per request, attach it to the active OpenTelemetry span as a `request.id` attribute, and a `ClientHttpRequestInterceptor` forwards it onto the core-service-to-AI-service RestClient call. Each metadata source's fetch failure and the OpenAI synthesis call's failure are now logged distinctly and routed through the Sentry capture path, rather than disappearing into the pipeline's generic swallowed `status="ERROR"` response. The Grafana dashboard (request rate, error rate, latency percentiles for both services) is checked in as a dashboard-as-code JSON file at `observability/grafana/hittiguess-overview-dashboard.json`, following Grafana's own dashboard JSON model, ready to import once a Grafana instance exists. Left undone: creating the actual Grafana Cloud account and Sentry project, pointing the built metrics/log/trace pipes at them, the live dashboard itself, uptime monitoring (no production deployment exists yet), and the free-tier usage-limit check (depends entirely on those accounts).

Why: building the account-independent instrumentation now, rather than deferring the whole story until account creation happens, means the moment a Grafana Cloud account and a Sentry project exist, they're pointed at working, tested instrumentation instead of becoming a second, larger implementation task. Every account-dependent task is bounded by a hard rule: agents don't create third-party accounts on the user's behalf. Defaulting every export path to a safe no-op absent explicit configuration means the instrumentation can be merged and run in every environment, including this sandbox's own test suite, without a live collector, Sentry project, or Grafana Cloud account ever being reachable from it.

---

## 2026-09 | Story 38's account-dependent pieces built: Grafana Alloy for metrics, OTLP for logs, two real bugs fixed

Decision: with real Grafana Cloud and Sentry accounts now created, the remaining account-dependent pieces of story 38 are built. Metrics ship through a new Grafana Alloy container (`observability/alloy/config.alloy`, added to `backend/docker-compose.yml`) rather than the application pushing them directly: neither service runs as its own container (`make dev` starts both as plain host processes), so Alloy scrapes `/actuator/prometheus` and `/metrics` through `host.docker.internal` and remote-writes the result to Grafana Cloud's hosted Prometheus. Logs ship through Grafana Cloud's unified OTLP gateway instead of a Loki-specific push path, the same endpoint and credentials traces already use: the core service gets a second Logback appender (`OpenTelemetryAppender`, from the newly added `opentelemetry-logback-appender-1.0`, which needed importing `opentelemetry-instrumentation-bom-alpha` alongside the existing stable BOM since the appender isn't part of the stable one), and the AI microservice gets an OTLP `LoggingHandler` from `opentelemetry-instrumentation-logging`.

Two real bugs surfaced while verifying this against live Grafana Cloud credentials, both AI-microservice-specific, and both fixed here. First, `tracing.py` passed `settings.otel_exporter_otlp_endpoint` directly to `OTLPSpanExporter`'s `endpoint` argument; doing so skips the OpenTelemetry SDK's own per-signal path resolution, which only runs when the SDK reads the endpoint from the environment itself. Every trace export was silently 404'ing against Grafana Cloud's gateway, confirmed by a direct HTTP request against the same endpoint with and without the `/v1/traces` suffix. The fix appends the correct suffix explicitly (`/v1/traces`, `/v1/logs`) rather than relying on environment-variable auto-detection, since `pydantic-settings` loads `.env` into its own `Settings` object, not into the real process environment, so the SDK's own env-reading path was never going to see it regardless. Second, the new logging bridge's handler was initially attached to the root logger, following the OpenTelemetry example almost verbatim; that immediately created an unbounded feedback loop, since the OTLP exporter's own HTTP calls log through `urllib3`, which the root-attached handler re-exports, triggering another HTTP call and another log line, forever. Reproduced directly (the process hung until killed) before fixing it by attaching the handler to the `"app"` logger specifically, the parent of every `logging.getLogger(__name__)` call in this codebase, which sits outside the namespace OpenTelemetry's and urllib3's own loggers use.

Why: Alloy over a direct in-app push avoids adding a second metrics-export code path per service when neither runs as a container Alloy could otherwise scrape without help; one shared agent doing the scraping keeps that logic out of application code entirely. Routing logs through the same OTLP gateway as traces, rather than standing up a separate Loki-specific credential and push path, means one less credential set to manage for a signal Grafana Cloud already accepts over OTLP. Both bugs were caught by testing against real Grafana Cloud credentials rather than trusting the code as written, confirming CLAUDE.md's own testing discipline extends to third-party integrations exactly where they're hardest to verify from unit tests alone: an HTTP call to a real external endpoint, and a runtime feedback loop that only manifests once a handler is actually attached and receiving real log volume.
...
Why: the project owner's own test cycle only needs three tiers, not phase numbers, to reason about: can they play it, can other people play it, is it done. Test coverage written mid-build gets rewritten every time the surface it covers changes, story 27's rate limiter and story 37's `DELETE /me` fix both landed real behavior changes after batch 18's tests were written against the pre-fix code. Writing it once, last, against a settled Local feature set costs less than maintaining it continuously through stories that haven't shipped yet. The design canvas predates this decision only in the sense that nobody had gone back to check it against `TASKS.md`'s stale unchecked boxes; the actual mockup work was already complete.

---

## 2026-09 | Story 44 built: TEST role, Spring profiles as the environment mechanism, startup guard instead of a CI check

Decision: story 44 introduces real Spring profiles (`spring.profiles.active`, wired to the existing `APP_ENV` variable) as the environment mechanism, rather than a hand-rolled property. `EnvironmentGuard.isProduction()` wraps `Environment.acceptsProfiles` and is the one place that defines what Production means; `TestRoleAccessGuard` and `TestAccountSeeder` both go through it. Every `TEST`-role guardrail this story adds only ever disables behavior when `EnvironmentGuard.isProduction()` is true, it never depends on a `TEST`-role row's mere absence. The "startup or CI check" task is built as an application startup guard (`ProductionTestAccountStartupGuard`), not a CI pipeline step: CI has no access to the real production database to check against, so a CI job could only ever check a database that isn't the one the task cares about. The seeded test account's credentials are documented in a new `docs/DEV_SETUP.md` rather than `CONTRIBUTING.md`, since story 36's `CONTRIBUTING.md` hasn't merged into `dev` yet; this content moves into `CONTRIBUTING.md` once that branch merges.

Why: `spring.profiles.active` and `@Profile` are Spring Boot's own supported mechanism for exactly this kind of environment-gated behavior, confirmed against the current Spring Boot 4.1 documentation rather than assumed from memory, and it composes with `Environment` and `@Profile` anywhere in the codebase instead of requiring every new check to re-read a custom property by hand. Reusing the existing `APP_ENV` variable instead of introducing a second, separately-configured environment signal keeps `CookieUtil`'s existing `app.env` check and the new profile check from ever disagreeing about which environment is active. A CI-pipeline version of the "fails loudly" check was considered and rejected: nothing in a CI run has a connection to the actual Production database, so the check that matters can only run where the application itself runs, at its own startup, against its own real connection.

---

## 2026-09 | Story 37 built: song attribution survives account deletion, legal pages are markdown for now

Decision: `Song.addedBy` becomes a nullable foreign key (`V11__make_song_added_by_nullable.sql`). Deleting an account clears `addedBy` to null on every song it submitted instead of blocking the deletion or deleting the songs; `SongDTO.addedBy` and `SongMapper.toDTO` are updated to allow and handle a null value. `deleteUser()` also now applies `leavePlaylist()`'s existing departure logic (delete a playlist the user was the last member of, promote the earliest-joined remaining member to owner if the user owned a playlist others still belong to) once per playlist the deleted user was in, factored into a shared `departFromPlaylist` helper. That helper explicitly deletes the departing membership row through `PlaylistMembershipRepository` rather than relying only on the parent collection's orphan removal: when the departing user's own account row is deleted in the same transaction, Hibernate's flush ordering can otherwise try to null the membership's not-null `user_id` column before it resolves that the row needs deleting outright, which fails the same way the original FK violation did. A new `GET /api/users/me/export` endpoint and `PersonalDataExportService`, separate from the existing playlist-content `ExportService`, return a full GDPR personal-data dump: account fields, every playlist membership, and every submitted song. The privacy policy and terms of service live as markdown at `docs/legal/privacy-policy.md` and `docs/legal/terms-of-service.md`; a cookie-consent notice stays out of scope until story 34 (first-party analytics) ships, since no third-party trackers exist to consent to yet.

Why: a song is already treated as independent of any playlist it belongs to (see the "Song deletion reversed" entry above); letting its submitter attribution survive that same way, rather than blocking account deletion or deleting the song, keeps that same principle consistent across both relationships. Reusing `leavePlaylist()`'s ownership-transfer logic for account deletion instead of inventing separate behavior keeps one rule for what happens when a user stops belonging to a playlist, regardless of why. The GDPR export endpoint stays a separate service rather than an extension of `ExportService`, since that service exists to generate printable playlist PDFs, a different concern that happens to share the word "export." The legal pages stay markdown rather than becoming real Next.js routes because this batch is backend-focused (the bug fix and the export endpoint); wiring them into the frontend's actual page structure is deferred to story 28's implementation phase, the same deferral pattern already used for other backend-first stories in this project.

---

## 2026-09 | Story 36 built: open-source collaboration readiness

Decision: `CODE_OF_CONDUCT.md` adapts the Contributor Covenant, version 2.1, rather than a custom document, with only the enforcement section rewritten for a single-maintainer project instead of the template's committee-based process; reports route to the maintainer's own email. `CONTRIBUTING.md`'s secrets section documents the three variables a new contributor needs sandbox-safe values for to run the app at all (`YOUTUBE_API_KEY`, `OPENAI_API_KEY`, `INTERNAL_SERVICE_API_KEY`), plus `DISCOGS_CONSUMER_KEY`/`DISCOGS_CONSUMER_SECRET` as a fourth, called out separately since it's a real production source (story 25, already shipped) rather than optional, it just degrades gracefully instead of blocking startup when unset. `ai/.env.example` lists several more variables than that: DeepInfra's client exists in code but nothing in the live `resolve_metadata` path calls it yet, story 18 hasn't wired it in, and the rest (Wikidata/Wikipedia bot logins, and the story 20 spike's other candidate providers) are explicitly commented in `ai/.env.example` itself as unused by the production microservice. Documenting setup instructions for those would describe accounts nobody actually needs today.

Why: the Contributor Covenant is the de facto standard for open-source codes of conduct; writing a custom one from scratch would trade a widely recognized document for a bespoke one with no real benefit at this project's scale. Scoping the secrets section to the variables a contributor actually needs, distinguishing "required to start" from "a real source that degrades gracefully" from "not called by production code at all," keeps `CONTRIBUTING.md` honest about what each one actually is, rather than either overstating a spike-only variable's importance or silently omitting a real one.

---

## 2026-09 | Story 27 built: Bucket4j for the core service, a hand-rolled sliding-window limiter for the AI microservice

Decision: the core service adds Bucket4j (`bucket4j-core`, framework-agnostic, no Spring dependency of its own) and applies time-window rate limiting through a single servlet filter, `RateLimitingFilter`, layered after `JwtAuthenticationFilter` in the security chain. Every request is keyed by authenticated user where one exists, by client socket address otherwise, at a general limit of 60 requests per minute. `/auth/login` and `/auth/register` are always keyed by socket address regardless of authentication state, sharing one stricter bucket at 5 requests per minute, since those are exactly the endpoints credential-stuffing and account-enumeration attempts target. The socket address, not `X-Forwarded-For`, is the key in both cases: this deployment has no reverse proxy in front of it to set that header honestly, so it is fully attacker-controlled, and keying on it would let an attacker bypass either limit by sending a different value on every request. This sits alongside `SongMetadataService`'s existing single-in-flight-request-per-user gate on `/api/metadata/song` rather than replacing it, the two solve different problems: one in-flight request at a time versus no more than N requests in a time window. Every 429 the core service returns, the filter's own and the existing in-flight gate's, now uses the app's `ErrorResponse` record; the in-flight gate's `RateLimitExceededException` replaces the `ResponseStatusException` it used to throw, which rendered as Spring's default `ProblemDetail` instead of the app's own error shape. The AI microservice gets an independent limit on `/metadata/resolve`, a hand-rolled in-memory sliding-window counter (`app/rate_limit.py`) keyed by client address at 30 requests per minute, evaluated before the `X-Internal-Api-Key` check so repeated attempts to guess that key are limited too. No new dependency (`slowapi` or otherwise) was needed for it, and no Redis or other shared store either: both services run as a single instance each, so per-process in-memory state is correct, the same reasoning already used for `SongMetadataService`'s in-flight set.

Why: Bucket4j is the standard choice for Spring rate limiting and has no Spring Boot 4 compatibility issues to check, since it depends on nothing from the framework at all. A single filter rather than per-controller annotations covers every public-facing endpoint uniformly and keeps the auth-specific limit and the general limit as one mechanism instead of two. Keying login and register by address unconditionally, rather than falling back to the general per-user/per-IP key logic, matters because an attacker enumerating accounts or stuffing credentials never has a valid session to be keyed by user in the first place. Trusting `X-Forwarded-For` without a trusted proxy actually setting it would have made the limiter worse than not having one for exactly the attack it exists to blunt, a real forwarded-header-aware fallback becomes worth adding once a specific hosting platform (stories 7/8) puts a real proxy in front of this deployment. Standardizing the 429 shape removes the one place in the app where a client had to handle two different error response formats for the same status code. The AI microservice's own limit exists because anyone holding the shared internal API key, not just the core service, can call `/metadata/resolve` directly, and the core service's per-user limiting offers that endpoint no protection at all in that case.

---

## 2026-09 | Story 33: analytics store is a second Postgres database, not a new store technology

Decision: the append-heavy analytics/event store story 42 already carved out gets provisioned as a second Postgres database, `analytics-db` alongside the existing `db` service in `docker-compose.yml`, rather than a dedicated time-series or event-store technology. It carries its own Flyway migration history under `db/analytics-migration`, run through a separate `DataSource`, `Flyway`, and `JdbcTemplate` wired in `AnalyticsDataSourceConfig`, fully independent of the core service's `spring.datasource`-bound datasource and its V1-V10 migration history. The schema is a single `analytics_events` table (event type, timestamp, JSONB payload), with a typed payload record per event kind in code rather than a dedicated column set per event type. Retention defaults to 180 days, configurable through `analytics.retention.days`, enforced by a daily `@Scheduled` sweep following `GroupExpirySweeper`'s existing shape. Nothing yet calls the write API; story 34 instruments the real event-producing call sites.

Why: this is a solo-developer hobby project already running Postgres through Testcontainers for every other integration test and through `docker-compose` for local development. A second Postgres database keeps both of those working paths unchanged, needs no new technology to operate, back up, or reason about, and still gives the isolation story 33's tests require: two independent connection pools and two independent Flyway histories, so an analytics write cannot touch or block the transactional database's schema or its rows. A dedicated time-series or event-store technology would add real operational and testing weight (a new container to run locally, a new client library, a new set of failure modes) for a project at this scale, with no concrete requirement here that Postgres can't satisfy. A single JSONB-payload table over one table per event type avoids a schema migration every time a new event type is added, at the cost of losing column-level constraints on payload shape, an acceptable trade for a store nothing queries yet.

---

## 2026-09 | Documentation consistency pass after story 38 (observability) merged

Decision: after PR #81 (story 38) merged, a full pass across `PROJECT_STATE.md`, `TASKS.md`, `ROADMAP.md`, and `ARCHITECTURE.md` found and fixed several pieces of drift unrelated to story 38 itself, caught while checking it. First, a real story-ID collision: `ARCHIVE.md` already had an archived "Story 47" (replacing `SongTag` with a genre field and adding real paper-size export), while `PROJECT_STATE.md`/`TASKS.md`/`ROADMAP.md` separately used ID 47 for a newer "naming consistency" story introduced by the roadmap-restructuring PR. The naming-consistency story is renumbered to 49 (the next free ID) rather than disturbing the already-shipped, already-archived genre story. Second, `ROADMAP.md`'s batch-plan checklist had batches 12, 13, 15, 16, 17, and 19 (stories 33, 27, 37, 36, 44, and 42/43) still unchecked despite each one's PR having merged into `dev` long before story 38's; batch 14 (story 38) is checked too, since the checklist tracks a batch's PR merging, not the underlying story reaching `Implemented`, and PR #81 has merged even though story 38 itself deliberately stays `Ready`. Third, story 39 (Group)'s `PROJECT_STATE.md` row still read bare "Ready" despite its backend being fully built on `feature/group`, the same deferred-frontend-tasks shape stories 10 and 11 already carry as "Implemented" with a caveat; promoted to match. Fourth, `ARCHITECTURE.md`'s "What's built"/"Not yet built" summary was badly stale, still describing Discogs, pgvector dedup, the playlist/song relational fix, rate limiting, and song search by link/keyword as unbuilt when all had already shipped; rewritten against the current `PROJECT_STATE.md`/`ARCHIVE.md` state.
...
## 2026-09 | Story 30 backend: the difficulty model lives in the core service, computed on the fly, with the play-data tier scaffolded

Decision: story 30's difficulty model is built in the core service under `org.dariusturcu.backend.difficulty`, not the AI microservice, because it reads core JPA entities (`Song`, and the story 10 `Round`/`Guess` data) and makes no external call, and because the core service owns the schema the play-derived signal aggregates over. It is computed on the fly with no schema change: the per-song play signal is a grouped aggregate query (`RoundRepository.aggregatePlacementStatsBySong`) over scored rounds, not a stored `SongDifficulty` table or view, so it stays current with no refresh job and needs no migration. `SYSTEM_REFERENCE.md` lists a planned `SongDifficulty` table; it is not needed for the on-the-fly model and is left unbuilt.

A song's difficulty is one score in `[0, 1]`, where lower is easier. `SongDifficultyScorer` blends two signals: the play-derived correct-placement fraction (an easier song is placed correctly on the timeline more often, so difficulty is the complement of that fraction) and a Wikidata sitelinks popularity proxy (a more widely covered song is more recognizable, so a higher sitelinks count maps to lower difficulty). Thin play history is blended with the sitelinks proxy in proportion to how many scored rounds exist, so one lucky or unlucky round does not decide a song's tier; a song with no signal at all sits at the neutral midpoint. Every weight and threshold is a named constant carrying an initial heuristic value, not one fitted against data. No real play data exists to fit them against, so they are the deliberate starting point to tune once real rounds accumulate.

Group difficulty follows the tier: easy takes the lowest individual predicted score so the least experienced player is protected, hard takes a plain average, medium takes the median, matching `GAME_DESIGN.md` and the earlier tiering decision. Difficulty-tuned selection (`DifficultyTunedSongSelector`) scores the verified catalog for a group's players, keeps the songs whose group score bands into the requested tier, and returns them capped at the target card count, reusing the existing `Song` and `Round` repositories. An empty catalog and a tier with no matching songs both return an empty list rather than failing.

The Wikidata sitelinks count is not persisted yet. It exists only in `ai/spikes/wikidata_spike.py`, not in the real metadata pipeline (`ai/app/metadata/`), and the AI microservice never alters schema, so persisting it is a story-23 change to `Song` and the metadata write path. The scorer reads it through an `Optional` seam and falls back to the neutral default while it is absent, so real values flow in with no scorer change once the column and its capture land. The international-scope filter that gates on the same count is deferred with it.

The personalized collaborative-filtering tier is scaffolded, not built. `PersonalizedDifficultyPredictor` is the plug point and `AggregateBaselinePredictor` is the shipped implementation, returning the per-song baseline for every player. A trained model, its retraining job, and the accuracy-versus-baseline monitoring check are deferred: they need enough accumulated `Guess` data to beat the baseline, which does not exist until real rounds are played over time, and inventing tuned values against nonexistent data would be dishonest. The Difficulty-Based generation endpoint, the Custom-mode endpoint, the `Playlist` public flag those depend on, and all frontend are also deferred; the scoring and selection core they call is what this slice builds.

Why: difficulty reads core-owned entities and needs no external call or LLM, so the core service is where it belongs, and computing it from existing rows avoids a table that would need its own refresh path to stay accurate. Naming every weight and threshold as an initial heuristic, rather than presenting fitted numbers, keeps the model honest about the absence of real play data to tune against, and the `Optional` sitelinks seam plus the predictor interface keep the deferred pieces (sitelinks persistence, the trained model) as clean attach points rather than half-wired code.

---

## 2026-09 | Betting is positional, not a blind right-or-wrong flag

Decision: a bet carries a position, the bettor's own chosen insertion index into their own timeline, staked at the same moment as the bet itself and validated in range the same way the active player's own placement already is. The bet only wins if the active player's placement was wrong and that stated position also turns out correct once the song is revealed, checked with the same `isPlacementCorrect` logic used for the active player, applied to the bettor's timeline instead. A wrong stated position discards the card exactly like a round with no bettor at all. `RoundRepository.tryAcceptBet` sets `bettor_player_id`, `bet_placed_at`, and the new `bet_position` column in the same atomic conditional UPDATE, so the position is claimed by the same race-winning bet rather than a separate step. A winning bettor receives only the card, never a bonus token, in every case.

Why: the original `GAME_DESIGN.md` text and the initial story 10 implementation described betting as a blind flag, accepted whenever the active player was wrong with no actual position guess involved, which under-specified what the project owner's own design mockups always showed: the bettor staking a token between two specific cards on their own timeline. This closes that gap rather than reversing a prior deliberate decision. The reveal mockup's "+1 token" reward for a correct bet is confirmed wrong and is not implemented; the only reward for a correct bet is the card.

---

## 2026-09 | Betting targets the active player's shared timeline, one bet per interval, not one bettor per round

Decision: the previous entry's model, one bettor per round staking a position on their own separate timeline, is wrong and is superseded by this entry. Every bet is placed against the ACTIVE PLAYER's own timeline, the same one the active player locks their own guess into and the one every other player is already looking at during that round. During the 15-second betting window, multiple players can each place a bet, at the same time as each other, as long as each picks a different gap in the active player's timeline than every other bettor and none of them pick the gap the active player already locked in. A round can end with zero, one, or several accepted bets, one per distinct gap; a player still can't bet twice in the same round, still can't bet if they're the active player or DJ, and still needs a token, deducted immediately on acceptance. A `Bet` entity (`round`, `player`, `position`, `placedAt`) replaces the single `bettorPlayer`/`betPosition`/`betPlacedAt` columns on `Round`; the `bets` table's `(round_id, position)` and `(round_id, player_id)` unique constraints are what make accepting a bet concurrency-safe now, in place of the single conditional `UPDATE` `RoundRepository.tryAcceptBet` used before: `BetRepository.insertBet` does a plain `INSERT`, and a unique-constraint violation, caught as `DataIntegrityViolationException`, means the bet wasn't accepted, the same outcome the old zero-row `UPDATE` produced. Since exactly one gap in the active player's timeline is objectively correct for the revealed release year, and no two bets share a gap, at most one bettor can ever be sitting on it. If the active player's placement was wrong and a bettor is on that gap, that bettor wins the card, not the active player; it lands on the WINNING BETTOR's own timeline, at wherever it objectively belongs there, computed automatically rather than chosen by the bettor. Every other accepted bettor loses their token for nothing.

Why: this was surfaced directly by the project owner reviewing the first version of this fix: "not one bet per round. one bet per interval, and you can't bet on the interval that the active player guessed," on the active player's own timeline specifically, "when a player is playing, everyone sees that player's timeline, and that's where the other people bet," confirming a rule already settled during the project's own design planning that the first pass had gotten wrong. Modeling bets as a list against a shared timeline, rather than a single column against each bettor's own, is what makes "several people can watch and bet on the same round" representable at all.

---

## 2026-09 | Story 40: playlist-link expansion lives in the AI microservice, progress is a per-user STOMP channel

Decision: expanding a YouTube playlist link into its video IDs (`playlistItems.list`) is built in the AI microservice, alongside the existing single-video YouTube integration, not in the core service. `fetch_playlist_video_ids` in `ai/app/metadata/sources/youtube.py` paginates through `nextPageToken`, and `POST /metadata/playlist-video-ids` exposes it internally, gated by the same `X-Internal-Api-Key` check `/metadata/resolve` already uses. The core service calls it through a new `PlaylistExpansionService`, which also owns merging the expanded IDs with whatever IDs or links a caller submitted alongside the link, deduplicated. Both catalog-seeding paths use it: `BulkImportService.importImmediately` and `CatalogSeedingService.enqueue`'s new `AdminCatalogSeedingRequest` overload. A link that doesn't parse to a valid playlist, or an upstream fetch failure, raises `PlaylistImportException`, mapped to a 400 by `GlobalExceptionHandler`, rather than the import silently proceeding as if the link had been ignored.

Live per-song progress for the on-the-spot bulk-import path (`ImportPlaylistProcessingDark.dc.html`'s per-row states) is a new per-user STOMP channel, not polling. `BulkImportService` publishes a `BulkImportProgressEvent` for every submitted video ID as its resolution loop reaches it; `BulkImportProgressListener` forwards each one to `/user/queue/bulk-import-progress` through `SimpMessagingTemplate#convertAndSendToUser`, the per-user counterpart to `GroupBroadcastListener` and `SessionBroadcastListener`'s per-group and per-session broadcasts. This is new ground for the STOMP layer: `WebSocketConfig` previously registered only `/topic` as a broker prefix, which routes broadcasts but not the `/queue`-prefixed destination Spring's `UserDestinationMessageHandler` rewrites a per-user send into, so `/queue` is now registered alongside it. The admin catalog-seeding backlog does not get this channel: `AdminCatalogBacklogDark.dc.html` shows a backlog-count and quota summary rather than live per-item progress, so its enqueue endpoint stays synchronous request-response.

Why: the AI microservice is the only place in the system with YouTube Data API access, so a second, redundant integration in the core service would duplicate what already exists rather than extend it. A dedicated `PlaylistExpansionService` keeps playlist-link handling in one place both catalog-seeding paths share, instead of each reimplementing the expand-and-merge step. Per-user STOMP delivery matches what a bulk import actually is, one user's own request, rather than reusing a broadcast pattern built for shared group or session state; polling was rejected because the mockup calls for rows updating live as each song resolves, not on a refresh interval.

---

## 2026-09 | Story 30 public playlists: publish/unpublish, browsing, and saving without membership

Decision: `Playlist` gets a `boolean isPublic` field, default false, added by migration `V15`. Publishing is two owner-only endpoints, `POST /api/playlists/{playlistId}/publish` and `POST /api/playlists/{playlistId}/unpublish`, rather than a single toggle: this controller already models every other owner-only state change (promote, kick, ban) as its own named action per existing state rather than a generic toggle, so publish/unpublish follows the same shape instead of introducing a different one next to it. Both call `PlaylistAccessService.requireOwner`, the same check `updatePlaylist` and the member-management endpoints already use.

`PlaylistAccessService.requireRead` itself is extended to pass for any playlist with `isPublic` true, regardless of ownership or membership, rather than adding a separate public-read path alongside it. Every existing playlist read, including story 45's import source check, already calls `requireRead`, so extending the one check makes a published playlist readable and importable everywhere that check already gates, with no separate wiring per call site. This also resolves story 45's deferred task: importing from a public playlist the requester neither owns nor is a member of now works, with no change to `PlaylistImportService` itself.

Browsing is `GET /api/playlists/public`, open to any authenticated user, returning every playlist with `isPublic` true via a new `PublicPlaylistSummaryDTO` (id, name, color, songCount, and the owner as a `UserSummaryDTO`, the same DTO `SongDTO.addedBy` already uses). `PlaylistSummaryDTO` is left alone rather than extended with owner info, since its existing callers (a user's own playlist list) have no use for it.

Saving a public playlist into another user's library is a new `SavedPlaylist` entity (`user`, `playlist`, `savedAt`), its own `saved_playlists` table with a unique constraint on `(user_id, playlist_id)`, deliberately independent of `PlaylistMembership` and `PlaylistBan`: no grants, no per-playlist identity, no kick/ban interaction. `POST`/`DELETE /api/playlists/{playlistId}/save` and `GET /api/users/me/saved-playlists` are open to any authenticated user. Saving rejects a non-public playlist, the caller's own playlist, and an already-saved playlist, all as `ConflictException`, matching how `PlaylistService` and `UserService` already surface rule violations (an already-existing membership, a target that's already the owner). An already-saved playlist is treated as a conflict rather than a silent no-op, consistent with `joinPlaylist`'s existing already-a-member conflict.

Why: reusing `requireOwner` and `requireRead` instead of new checks keeps public playlists inside the access model every other playlist operation already goes through, rather than adding a parallel one. Modeling saving as its own entity instead of a flag on `PlaylistMembership` or a degenerate membership row keeps the two concepts, a bookmark versus real access, from ever being confused at the code level. This is a narrow, independently-decided slice of story 30, built ahead of the rest of that story, which stays open pending the difficulty-generation scope; see `PROJECT_STATE.md`'s story 30 row.

---

## 2026-09 | Admin report resolution can override a locked song, superseding the uphold-only lock rule

Decision: `POST /api/admin/song-reports/{songId}/uphold` is replaced by `POST /api/admin/song-reports/{songId}/resolve`, taking a `ResolveReportRequest` (`correctedYear`, nullable, and `verificationStatus`, required, one of `VERIFIED`/`NEEDS_REVIEW`/`MANUAL_ENTRY`). Resolving a song's open reports marks them upheld and applies the admin's chosen year and status unconditionally, including on a song currently `VERIFIED`. This supersedes the 2026-09 "Story 17 report resolution: uphold semantics without a re-resolution pipeline" entry's rule that a locked song's year and status stay unchanged when its reports are upheld, and narrows the "once locked, no code path may overwrite the year" rule from the story 18 lock-evaluation entries to exclude this one specific admin-driven path. Every other path, including a report simply being upheld with no explicit admin override, still respects the lock: `EDITABLE_VERIFICATION_STATUSES` is removed from `SongReportService` entirely since the resolve path no longer checks it and nothing else in that class needs it.

Why: `docs/design/source/AdminReportQueueDark.dc.html`'s admin report-resolution screen shows a song explicitly labeled "currently locked at 1983, VERIFIED" with an editable year field and a verification-status dropdown submitted together through a single "Resolve" action, and the project owner confirmed directly, after this contradiction with the standing lock rule was raised, that the design is the correct one. An admin manually resolving a report is the deliberate human review step the lock rule was always meant to route disputed `VERIFIED` songs into, story 40's automated re-resolution pipeline does not exist to do it any other way, so the lock has to yield to that one explicit, admin-driven action rather than leaving a correctly-identified wrong year permanently stuck.

---

## 2026-09 | ROADMAP.md's batch plan retargeted to story 28's screen clusters

Decision: `ROADMAP.md`'s "Batch plan" section, originally one story per PR sequencing Phase 1 and Phase 2, is replaced rather than removed outright or left as a stale checklist. Nearly every story it sequenced has since shipped, several out of the order it originally picked, through branches it never named, so the old list no longer describes how to reach for the next unit of work. The only piece of genuinely sequential work left toward Local is story 28's implementation phase, so the new batch plan sequences that instead, split into the same screen clusters `docs/design/hittiguess-design.html` and `FRONTEND_CONTENT.md` already group its 53 mockups into (existing pages, planned playlist screens, song review and search, admin views, gameplay screens, and a final cross-cutting batch for the component/token boundary and accessibility pass).

Why: a batch plan earns its place by giving a session a starting point without re-deriving one; a plan describing finished work no longer does that, and deleting the section entirely would lose a genuinely useful pattern, one ordered PR-sized list, for the one sequential stretch of work that remains. Story 28 is that stretch: its own tasks in `TASKS.md` are just two broad implementation bullets with no PR-sized breakdown, so retargeting the batch plan section is where that breakdown belongs, grouped by the same screen categories the design and content docs already use rather than inventing a new taxonomy.
...
`totpSecret` is a plain, unencrypted column on `User`. This codebase has no existing encryption-at-rest primitive for any column (`JWT_SECRET`, refresh tokens, and password hashes all rely on hashing or transport security, never column-level encryption), so adding one here would be new infrastructure built for a single field rather than a project-wide pattern, disproportionate to this story's scope. The safeguard is that no DTO or API response ever serializes `totpSecret` (covered by its own test), the same boundary `RefreshToken`'s hashed token and `User.password` already rely on. A database compromise that reaches `totpSecret` already reaches `password` and every refresh token, so this column is not a materially weaker link than what already sits next to it.

The intermediate two-factor login token is a JWT signed with the same key as the real access/refresh tokens, carrying `userId` and a `tokenType: "two_factor_pending"` claim, five minutes' expiration (`jwt.two-factor-pending-expiration`), issued by `JwtUtil.generateTwoFactorPendingToken`. It is never accepted by `JwtAuthenticationFilter`, which only reads the `access_token` cookie and skips `/auth/**` entirely apart from the three endpoints that need a real session (`/auth/2fa/setup`, `/confirm`, `/disable`); `AuthService.verifyTwoFactor` is the only code path that ever inspects a pending token, and rejects one missing the `tokenType` claim outright. A new claim on the same JWT shape was chosen over a second token store (a database-backed pending-login table) because the pending token is short-lived and stateless by nature, an intermediate step that either completes or expires, with nothing worth persisting if it never gets used.

`/auth/resend-verification`, `/auth/password-reset/request`, and `/auth/2fa/verify` were added to `RateLimitingFilter`'s existing IP-only strict bucket alongside `/auth/login` and `/auth/register`. The task list named only the first explicitly, but a second-factor code is a 6-digit brute-force target and a password-reset request sends a real email on every call; both fit the same rationale the login/register bucket already exists for, and leaving them on the general 60-per-minute limit would be a real gap in an "auth hardening" story.

Why: each of these is a real fork in the code that the task list left to judgment rather than a change the task list already settled. Blocking login outright is the simplest rule that actually stops an unverified account from doing anything, and pairing it with no-auto-login-on-register closes the one path that would have quietly defeated it. Plain-column `totpSecret` matches the security posture already implicit in the rest of the schema instead of introducing a one-off encryption scheme with no other column to share it with. A claim on the existing JWT keeps the two-factor login step stateless and consistent with how access and refresh tokens already work, rather than adding a new table and cleanup job for a value that lives for at most five minutes.

---

## 2026-09 | Story 28's retheme-only rule is superseded for pages whose mockup layout differs from the existing page
...
## 2026-09 | Story 30 sitelinks persistence: nullable count column, separate count fetch, unknown stays null

Decision: `songs` gains a nullable `wikidata_sitelinks_count` integer column (`V21`), populated by both song persistence paths (`SongResolutionService` for pipeline-resolved songs, `PlaylistService` for user-confirmed creations). The AI microservice resolves the count with a dedicated `wbgetentities` call asking for `sitelinks` only, against the track candidate's entity id, rather than widening the existing labels and claims lookup, and threads it through `SongMetadataResult` into `AiMetadataContent` and `SongMetadataResponse`. A failed lookup resolves to null, never zero, and the difficulty scorer treats null as the neutral midpoint. The international-scope cutoff stays undecided until real catalog data exists to check a value against; the scorer's existing minima keep their heuristic values.

Why: a sitelinks payload carries every linked edition's full entry, so folding it into the lookup that runs for every candidate would bloat a call whose result is otherwise small, while the count is only ever needed for the single track entity. Null for unknown keeps a song with no capture history (everything resolved before this column existed, plus any failed lookup) from skewing easy or hard; zero would read as maximally obscure, which is a claim about the song, not a statement that nothing is known. Persisting through both creation paths matters because user-confirmed songs bypass the resolution service entirely.

## 2026-09 | Story 30 session starts: preview-then-confirm generation, staged pools, admin-only

Decision: Difficulty-Based starts split into a side-effect-free preview (`POST /api/groups/{groupId}/session/generate`, tier plus target count in, song previews out) and a confirm (`POST .../start-with-songs`, reviewed ids in), with Custom-mode (`POST .../start-custom`, exactly one of a playlist id or a pasted playlist link) beside them. All three are group-admin-only, matching the legacy start. The preview returns identity and release facts only, never the youtubeId the round flow withholds until the reveal. The confirmed or custom song ids reach the session through an in-memory staged pool keyed by groupId (`PendingSessionSongPool`, mirroring `SessionResultsStore`): the endpoint stages the pool, then runs the unchanged group lock and broadcast, and the existing start listener consumes the staged pool exactly once when the broadcast fires. A start that fails before the broadcast discards its staged pool, so nothing leaks into a later start. A pasted playlist link expands to video ids and maps each to its catalog song, skipping videos with no match; a link with no matches at all is a conflict, not an empty session. The international-scope cutoff for generation is 5 sitelinks, an initial heuristic to tune once real catalog data exists.

Why: previewing without starting is what the review step needs, and revalidating the confirmed ids on the way in keeps a stale or hand-edited review from starting a broken session. Routing through the existing lock and broadcast keeps every client in sync exactly the way the legacy start does, instead of growing a second session-creation flow with its own sync semantics. Withholding youtubeId from the preview respects the same rule that keeps it out of round state before the reveal. Skipping unknown videos rather than failing the whole link treats a playlist with some uncatalogued tracks as usable, while failing on zero matches tells the admin plainly there is nothing playable.

## 2026-09 | Pixel-art image rule: pixelized uploads stored as database bytes
...
## 2026-09 | The AI service installs from a uv lockfile

Decision: `ai/uv.lock`, generated by `uv lock`, pins every direct and transitive dependency of the AI service, including the dev extra. CI installs with `uv sync --locked --extra dev` and runs the tests through `uv run --locked`, so a lockfile out of step with `pyproject.toml` fails the build instead of resolving fresh versions. `pyproject.toml` keeps its version ranges, and `uv lock` is rerun whenever one changes.

Why: the service had only unpinned `>=` ranges and no lockfile, so every install could resolve different versions, builds weren't reproducible, and there was no fixed set of versions to audit. uv reads the existing `pyproject.toml` unchanged and creates the same `.venv` the Makefile already runs from.

## 2026-09 | Game sessions recover from restarts and no longer stall

Decision: a player counts as connected while any of their sockets is subscribed to the session's round topic, and subscribing marks them connected again. A session needs at least players times the win condition in songs to start, and when the queue runs out it completes on the current standings. Each round has a three-minute placement deadline; an idle active player's card is discarded like a wrong placement with no bets, without marking them Left. Skip betting is open only to eligible bettors, and the window closes once every eligible bettor holding a token has bet or skipped. On startup, each in-progress session's current round timer is rescheduled from the round's stored timestamps, every player starts disconnected until a socket subscribes again, and the auto-abandon timer is armed. Completed session results are stored in the session_results table. A guess's result goes only to the guesser, and a player's first correct artist or title is broadcast without the answer.

Why: the reconnect path had no caller and every socket close counted as a disconnect, so a reload got an active player marked Left and could abandon a live session. An empty queue threw inside the scheduled advance and left clients on the reveal. Timers and results lived only in memory, so a restart stranded rounds. An idle but connected active player held the round forever, and any single player could end everyone's betting window. The placement deadline is generous because the active player listens through YouTube's ads first. The song minimum gives every player enough turns for a game with good placements to reach the win, and matches the set size the lobby already generates for difficulty starts. The skip rule follows GAME_DESIGN.md: the button ends the window early when no one wants to bet.

## 2026-09 | Shared songs, covers, member identity, and results are access-checked

Decision: a song can be edited directly only by someone who can write every playlist holding it; anyone else reports a correction instead. A playlist's custom cover needs the same read access as the playlist. A member's per-group display name is 1 to 30 characters with no control characters, and a per-group avatar URL must be an https Google profile image. A group's stored session results can be read only by the users who played that session.

Why: a song row is shared across playlists, so one playlist's writer could change a song everywhere it's used. The cover endpoint was public, exposing private playlists' covers by id. Every member's browser loads a member's avatar URL, and Google profile images are the only avatar URLs the app produces, so any other host would only serve tracking or abuse. Group ids are sequential, so an unchecked results endpoint let anyone read any group's results.

## 2026-09 | Imports are capped and metered, and group admins can remove members

Decision: an import holds at most 200 songs, on both the on-the-spot bulk import and the background playlist import. Each user gets 500 new songs resolved per UTC day across both paths, counted in the import_quota_usage table and reserved in one statement before any paid lookup starts; songs already in the catalog don't count. The bulk import checks write access to its target playlist before any lookup. Uploaded images are refused from their header's declared dimensions before any pixel data is decoded. Join codes allow ten attempts per user per ten minutes. A group admin can remove another member while no game session runs; the removed user can't rejoin that group, and their open sockets are closed after the removal commits.
...
## 2026-09 | Rounds are full passes, games end at a round's end, and guessing is one shot per field

Decision: a round is one full pass through the active-player rotation; each rounds row is a turn inside it, and the round number shown to players advances once the rotation reaches a player who already had a turn in the current round. Reaching the win condition no longer ends the game on the spot: the game completes when the round in which anyone reached it closes, and all results use competition ranking, so players tied on cards, artists guessed, or titles guessed share a place. Players start with two tokens. The title gets one guess per turn. Artists are guessed one at a time: a correct artist counts and allows another, a wrong one closes artist guessing for the turn, and repeating an artist already guessed is refused. The active player earns one token per turn for the title plus at least one artist. When a session ends, the ended event carries the group id, the group broadcasts that it's open again, and the results screen reads the group from its URL. This supersedes the "fully correct guess, both artist and title" token rule and the single-turn round of the earlier gameplay entries.

Why: from playtest feedback. Ending mid-round gave players earlier in the turn order an extra turn over the rest, and it ruled out ties. Songs with several credited artists made "every artist" nearly unreachable, and unlimited resubmissions turned guessing into brute force. Sessions are purged when they end, so a client that learned about the end by refetching the session got a 404 and was stuck on it.

## 2026-09 | LAN playtests with voice run behind a local HTTPS proxy, and the DJ's audio share starts from the YouTube click

Decision: a LAN playtest that needs voice runs `scripts/lan-https-proxy.mjs`, a dependency-free Node proxy that serves one HTTPS origin on the LAN address with a keytool-generated self-signed certificate and forwards backend paths (`/api/`, `/auth/`, `/ws`, the OAuth handshake, API docs) to the Spring Boot service and everything else to the Next dev server. The DJ's "Open on YouTube to play" click requests the tab-audio capture first and then opens YouTube in its own named window with the opener link cut; phones keep the plain link to the YouTube app. A player's signalling socket connects before the join is announced. The voice sidebar keeps the left sidebar's 76px width in every state and shows only on the group lobby page or while its player is in the call.

Why: browsers expose `getUserMedia`, `getDisplayMedia`, and the Clipboard API only on secure origins, so voice and audio sharing could never work over the plain-HTTP LAN address. One proxy origin means one certificate to accept, no cross-origin API calls, and no TLS setup in either service. The capture request needs the click's user activation, so it goes before the window opens, and a separate window keeps the picker visible in the game window. Announcing the join first let other members send their offers before the joining socket was listening, so the first connection attempt was always lost and voice waited on the eight-second retry.

## 2026-09 | Every user YouTube import runs on the fast tier, in parallel, and the patient tier rechecks it

Decision: every user import of YouTube videos, the background playlist import and the on-the-spot bulk import alike and whatever its size, resolves new songs on the fast tier from the earlier two-tier entry, with no size threshold. The AI service exposes it as two calls: `/metadata/identify` (YouTube fetch, hard filter, verified-duplicate check, and the pre-check LLM call) and `/metadata/date-fast` (a provisional year from one lane, MusicBrainz or Wikipedia plus LLM extraction, whichever frees up first given the work in flight, falling back to the other lane when the first finds nothing). The backend dispatches every new song's identify call at once onto a bounded pool, and each song moves to its year lookup the moment its own identify returns, so no song waits on another. Each source's pacing in the AI service reserves request slots under a lock, so parallel songs queue at the source's documented pace instead of bursting. A resolved song joins the playlist immediately and is saved UNVERIFIED, then queued as a `FAST_TIER_RECHECK` backlog row carrying its provisional year; the patient drain records its own year beside it, and the admin backlog lists both with a run-now drain. A song neither lane can date goes through the full pipeline inline and isn't rechecked. A manually added song is queued as a `USER_ADD_RECHECK` the same way. Every existing limit still holds: the backend paces its own fast-tier calls to the AI service's per-minute cap, the full-pipeline fallback runs one song at a time to stay inside `/metadata/resolve`'s 30 a minute, a source's cooldown or back-off holds its shared pacer for every caller, the import page polls no faster than every five seconds so one user stays inside the core service's 60 requests a minute, and the 200-song import cap and 500-song daily quota are checked before any of it starts. Test account 1 is seeded as an ADMIN. This supersedes the 2026-09 entry's claim that the fast tier was built into the microservice: until now only the patient pipeline existed, and user imports ran it one song at a time and then again from the backlog.

Why: owner decision after the second playtest, where a playlist import took minutes because every song ran the full patient pipeline sequentially, and each was then run through the same pipeline a second time from the backlog. The fast tier's spike scored 90% at a fraction of the calls, and the patient recheck catches the rest without the user waiting on it. Linking each song as it resolves lets a user start playing before a large import finishes.
