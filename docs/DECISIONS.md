...
Decision: the frontend sends a CSP that permits the API origin and matching WebSocket origin, YouTube thumbnails, Google avatars, self-hosted fonts, browser media blobs, and Next inline scripts. Production disables Springdoc. The AI service rejects an empty internal key at startup, uses a constant-time comparison, and exposes neither docs nor metrics. CSV formula prefixes are escaped with an apostrophe. Group joins lock the matching group row pessimistically before capacity is checked.
