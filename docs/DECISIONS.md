...
## 2026-09 | Story 22 batch: backend controller test approach, frontend test runner choice, Playwright deferred

Decision: the backend controller tests use standalone `MockMvc` (`MockMvcBuilders.standaloneSetup`) with the service layer mocked and, where the controller relies on it, `GlobalExceptionHandler` attached directly, rather than Spring Boot's `@WebMvcTest` slice. `@WebMvcTest` still boots the app's security auto-configuration, which builds `JwtAuthenticationFilter`'s dependency chain including the OAuth2 client beans; this development sandbox's JDK can't construct the `java.net.http.HttpClient` those need, the same limitation already documented on `SongSearchIntegrationTest`. JWT authentication, refresh-token rotation, and CSRF instead get their own dedicated real-filter-chain integration test (`JwtRefreshAndCsrfSecurityIntegrationTest`), built the same way `SongSearchIntegrationTest` builds a minimal security context rather than loading the whole application. `GameActionController`, the STOMP message controller, got a plain Mockito unit test instead, since MockMvc doesn't exercise STOMP mappings.

The frontend unit test runner is Vitest, not Jest, per current official Next.js 16 documentation's own recommended setup (`vitest`, `@vitejs/plugin-react`, `jsdom`, `@testing-library/react`, `@testing-library/dom`). Installing `@vitejs/plugin-react` against this project's pinned `shadcn` devDependency produced a real peer-dependency conflict: `@vitejs/plugin-react`'s optional `@rolldown/plugin-babel` peer wants `@babel/core@^8`, while `shadcn`'s own dependency chain needs `@babel/core@^7`. `npm install --legacy-peer-deps` resolves it; both `npm install` and `npm ci` in `.github/workflows/pr-checks.yml` now carry that flag for the frontend job. `vite-tsconfig-paths`, the plugin the official docs suggest for path-alias resolution, was replaced with Vite's now-native `resolve.tsconfigPaths: true` option instead, since the version installed here already supports it directly and flagged the plugin as redundant.

Playwright end-to-end setup, and the four core-flow tests (login/register, playlist CRUD, song add/edit, export) it would cover, are not started this batch. They need the full stack, frontend, backend, AI microservice, and a Postgres instance, running together, and this batch's session shared its development machine with a concurrent session working on story 44 against its own Postgres container; standing up a second full stack alongside that without a clear isolation story was judged too likely to collide with that other session's state to attempt safely in the time available. This is left as explicit follow-up work, not silently dropped.

Why: matching an established, working pattern already in the codebase (the minimal-security-context approach `SongSearchIntegrationTest` set) is lower-risk than introducing a new test-slice technique that hits the same known sandbox limitation and would need its own workaround anyway. Following the framework's own current documentation for the Vitest setup, rather than a remembered or assumed configuration, is exactly what `CLAUDE.md`'s tooling rule asks for on a project that pins bleeding-edge dependency versions; the peer-dependency conflict encountered while installing it is a concrete example of why that rule exists. Playwright is the single largest, most infrastructure-dependent piece of this story; rushing it under session-sharing risk would have jeopardized the correctness of the other, more tractable work this batch completed, so it was cut instead.
