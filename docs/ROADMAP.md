...
Per `AGENTS.md`'s batching workflow: open a PR per batch as each is finished, several may be open at once, and the project owner reviews and merges them asynchronously. A batch's PR being open does not block starting or opening the next; each batch's branch is cut from the current `dev` with `dev` synced into it before its PR opens.
