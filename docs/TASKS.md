...
Checked against real code: `ai/app/clients/openai_client.py` is the only LLM client, a module-level `OpenAI` singleton, no other client exists. The exploration pass below concluded and validated a model choice (gpt-5-nano for reconciliation, DeepSeek-V4-Flash for Wikipedia extraction) and a two-tier pipeline shape against real data, this is no longer an open "which model" question. What's still genuinely undecided, not just unwritten, is the one remaining item in that spike section: whether this validated design is worth building into the real AI microservice at all, versus staying validated-but-unbuilt. No implementation tasks are drafted here yet because that go/no-go call hasn't been made, not because the model choice itself is still unknown.
