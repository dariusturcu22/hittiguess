...
Checked against current code: the four sources (`musicbrainz.py`, `discogs.py`, `wikidata.py`, `wikipedia.py`) are fully implemented and wired into `_gather_all_metadata` (`ai/app/metadata/service.py`), each a synchronous function using synchronous `httpx` through `get_with_backoff`. The earlier note that the source set was "mostly-stubbed" is stale: story 25 (Discogs) shipped and the sourcing spike's MusicBrainz, Wikidata, and Wikipedia are all built and validated. `resolve_metadata` fetches all four sources in one pass and then runs a single synthesis call; there is no lock-gate or conditional Wikipedia branch in this code, so all four fetches are independent and parallelize together. Each source keeps its own real, already-validated rate limiter (MusicBrainz's and Discogs' adaptive limiters, Wikidata's and Wikipedia's documented per-minute limits): parallelizing runs the four concurrently against each other, per-source pacing stays in effect underneath, this story only removes the artificial serialization between sources.

Since the source functions are synchronous and `resolve_metadata` is a synchronous function that Starlette already runs in a worker thread, the concurrency primitive is a `ThreadPoolExecutor`, not an async rewrite: the four calls are network-bound, so one worker per source bounds wall-clock cost by the slowest single source instead of their sum, with no change to the source modules, their rate limiters, or their error handling. An async conversion would mean rewriting all four modules, `get_with_backoff`, and both adaptive rate limiters to async with no functional gain here, so it is not done.

- [x] Run the source fetches concurrently in `_gather_all_metadata` with a `ThreadPoolExecutor`, one worker per source, bounded by a named worker-count constant; each source's own adaptive or documented rate limiter stays in effect underneath the concurrency
...
Tests:
- [x] Unit test confirming the four sources are fetched concurrently, not sequentially (controlled per-source delay, asserts total elapsed is bounded well under the sequential sum)
- [x] Unit test confirming one source raising does not prevent the others' results from reaching synthesis
- [x] Unit test confirming all four sources are fetched and passed through to the synthesis step
