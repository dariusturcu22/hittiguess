...
- [ ] Opening settings, tier popup, custom picker, or chat closes the other three
- [ ] Starting a session closes all four popups
- [ ] Chat, settings, and tier popups share one footer gap distance

Voice mesh error text (`frontend/hooks/use-voice-mesh.ts`):
- [ ] Missing mediaDevices reports an HTTPS or localhost message, not the permission one
- [ ] getUserMedia catch branches on error name for distinct messages

Export redesign (backend `CardGenerator`, `ExportService`, `ExportController`, `PlaylistContent.tsx`):
- [ ] Combined mode removed on backend and frontend
- [ ] Single PDF interleaved duplex mode added on backend with frontend option
- [ ] Export dialog pickers use shadcn Select instead of raw select
- [ ] Empty playlist export shows an error toast instead of opening the dialog

Icon drag and avatar selection (global):
- [ ] Icon images not natively draggable, avatar initials not text selectable

Explore listing (`PlaylistService.getPublicPlaylists`):
- [ ] Owned and joined playlists excluded from explore results, saved unaffected

Invite preview error states:
- [ ] Dead playlist invite renders an invalid link screen instead of the join form
- [ ] Backend group invite preview endpoint mirroring the playlist one
- [ ] Group join page gates on the preview error state the same way

Import progress:
- [ ] YouTube import page shows inline crawling progress with per song checklist and no auto redirect
- [ ] Playlist songs refetch incrementally as import items finish
- [ ] Sidebar import indicator links to the YouTube import progress page
- [ ] Playlist page importing banner revisited once the above land
- [ ] From playlist import page spot checked against its mockup step

Members trigger (`PlaylistContent.tsx`):
- [ ] Trigger renders bare avatar stack with no box or label text, popup content unchanged

Featured artist extraction (AI prompt plus Java mapping):
- [ ] Prompt extracts featured credits into the artist field instead of embedding them
- [ ] Java mapping splits credits into MAIN and FEATURED SongArtist rows

Report dialog validation:
- [ ] Light validation pass on the report fields

Cover mosaic letterboxing (`playlist-cover-mosaic.tsx`):
- [ ] Tiles scale images past baked in bars in mosaic and single cover paths

Tests:
- [ ] Frontend tests for popup exclusion, export dialog, invite error states, import progress, members trigger, report validation
- [ ] Backend tests for explore filtering, group invite preview, duplex interleave, artist role split
- [ ] AI prompt or schema tests if the suite covers them
- [ ] Frontend `npm run test -- --run`, `npm run lint`, and `npm run build` clean
