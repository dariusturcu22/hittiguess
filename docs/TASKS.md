...
- [ ] `GlobalExceptionHandler`'s catch-all `RuntimeException` handler matches `ResponseStatusException` before Spring's own resolver gets to honor the status code the exception actually carries, so every `ResponseStatusException` thrown anywhere in the app (`SecurityUtils`'s 401/500 cases, `SongMetadataService`'s 429, `ExportService`'s 400) comes back to the client as a flat 400 regardless of what was actually thrown. Confirmed live in `SongMetadataControllerTest.getSongMetadataOnRateLimitFallsThroughTheCatchAllRuntimeExceptionHandler`. Add a dedicated `@ExceptionHandler(ResponseStatusException.class)` that reads and honors the exception's own status, ordered ahead of the generic `RuntimeException` handler.
