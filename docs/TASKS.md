...
- [ ] Custom button matches tier pills in row, styling, and aria-pressed treatment
- [ ] Playlist source state tracks tier vs custom and drives the active highlight
- [ ] Cards count input removed, target count computed as memberCount * winCondition * 3 clamped to minimum
- [ ] Custom picker modal gets vertical margin so the backdrop stays clickable, backdrop click closes it
- [ ] Playlist link input, Start button, and playlistLink state removed, startCustom usage cleared
- [ ] Picker rows render compact playlist cards reusing the library cover pattern
- [ ] Confirm button saves selection only, no session start

Chat overlay (`frontend/components/group-chat-overlay.tsx`):
- [ ] Backdrop click closes the panel in floating and non-floating paths, Chat toggle stays usable

Invite and OAuth return path:
- [ ] Login and register Google links carry validated returnTo
- [ ] Backend passes returnTo through the OAuth2 request and appends it to the frontend redirect
- [ ] RedirectHandler routes to validated returnTo instead of fixed /playlists
- [ ] Playlist join page carries the same returnTo login link as the group join page
- [ ] Logged out group join route checked for real redirect or query race vs dev only noise

Password fields:
- [ ] Reusable PasswordInput wrapper with Eye and EyeOff toggle
- [ ] Login, register, and reset password fields use the wrapper with existing bindings intact

Tests:
- [ ] Lobby tests for custom highlight, confirm without start, and backdrop close
- [ ] Chat test for click outside close
- [ ] Frontend tests for returnTo on Google links and RedirectHandler routing
- [ ] Backend tests for returnTo propagation through the OAuth handlers
- [ ] Password toggle reveal and hide test
