...
- [x] Data migration for existing memberships: default all three grants to true, owner determined by whichever user the migration treats as creator (decide the exact rule against real data, today's schema has no creator field to read from)
...
- [x] Integration test: the data migration assigns every existing membership full grants and a determinable owner, with no playlist left without one
