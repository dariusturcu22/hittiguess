...
- Story 30: a country/language filter dimension is dropped. A difficulty-generated set defaults to international scope instead, using Wikidata's sitelinks count (already validated during the metadata-sourcing spike) as the signal. Playlist selection is exactly two top-level modes, Difficulty-Based (Auto-Generated) and Custom (a playlist the player owns, is a member of, or that's published publicly, or one pasted directly); theme-request generation is dropped, publishing a playlist publicly stays a real capability, see the 2026-09 `DECISIONS.md` entry.
