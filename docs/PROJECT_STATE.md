...
| 20 | Local LLM option for lower-cost bulk metadata processing | AI | Needs Definition, model choice and pipeline shape decided and validated against real data (`DECISIONS.md`): gpt-5-nano for reconciliation, DeepSeek-V4-Flash for Wikipedia extraction. Still genuinely open, not just unwritten: whether to build this into the real AI microservice at all, versus leaving it validated-but-unbuilt (`DECISIONS.md`'s "Metadata pipeline final shape" entry). If greenlit, also needs story 23's schema and the same spike-handoff implementation as story 40 |
...
| 42 | Explicit database split: the domain boundary between the core transactional Postgres+pgvector database and story 33's separate analytics/event store | Infra | Ready, tasks checked against the real code and the current docs, not blocked |
| 43 | Metadata minimization: curb raw pipeline/metadata storage growth everywhere it's persisted, not just at story 23/40's specific fields | Backend / AI | Ready, tasks checked against the real code, not blocked |
| 44 | Test user infrastructure (dev only): a dedicated `TEST` role and reusable seeded credentials for automated agents, disabled outright in Production | Quality / Backend | Ready, tasks checked against the real code, not blocked; no environment/profile mechanism exists today, building one is now part of this story's own scope |
