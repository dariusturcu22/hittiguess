...
| 20 | LLM client infrastructure for the metadata pipeline's reconciliation/extraction models, narrowed from the original "local LLM option" framing | AI | Ready, greenlit to build (`DECISIONS.md`), tasks checked against real code, not blocked: a DeepInfra client for DeepSeek-V4-Flash, gpt-5-nano already reachable through the existing OpenAI client |
