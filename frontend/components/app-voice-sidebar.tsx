...
export function AppVoiceSidebar() {
  ...
  return <aside className="flex w-[100px] shrink-0 flex-col items-center border-l-[3px] border-sidebar-border bg-sidebar/95 px-2 py-[22px] backdrop-blur"><div className="flex flex-col items-center gap-6">{voiceMembers.map((member, index) => <div key={member.id} className="flex flex-col items-center gap-2"><div className={`flex size-[52px] items-center justify-center rounded-full font-display text-base text-primary-foreground ${MEMBER_COLORS[index % MEMBER_COLORS.length]}`}>{member.displayName?.charAt(0).toUpperCase() ?? "?"}</div><span className="max-w-[84px] truncate text-[10px] text-sidebar-foreground">{member.displayName ?? "Player"}</span></div>)}</div><div className="flex-1" />{microphoneError || voiceMesh.microphoneError ? <p className="mb-2 text-center text-[9px] text-destructive">Microphone permission is needed.</p> : null}<div className="flex flex-col items-center gap-2.5">{isInVoice ? <><button type="button" title="Mute" aria-label="Mute" onClick={toggleMute} className="flex size-10 cursor-pointer items-center justify-center rounded-xl bg-card text-card-foreground focus-visible:ring-2 focus-visible:ring-ring">{voiceMesh.isMuted ? <MicOff className="size-[19px]" /> : <Mic className="size-[19px]" />}</button><button type="button" title="Deafen" aria-label="Deafen" onClick={toggleDeafen} className={`flex size-10 cursor-pointer items-center justify-center rounded-xl focus-visible:ring-2 focus-visible:ring-ring ${voiceMesh.isDeafened ? "bg-primary text-primary-foreground" : "bg-card text-card-foreground"}`}><Headphones className="size-[19px]" /></button><div className="my-1 h-0.5 w-8 bg-sidebar-border" /><button type="button" title="Leave voice" aria-label="Leave voice" onClick={leaveCall} disabled={leaveVoice.isPending} className="flex size-10 cursor-pointer items-center justify-center rounded-xl bg-destructive text-destructive-foreground focus-visible:ring-2 focus-visible:ring-ring disabled:cursor-not-allowed">{leaveVoice.isPending ? <Loader2 className="size-4 animate-spin" /> : <PhoneOff className="size-[19px]" />}</button></> : <button type="button" title="Join call" aria-label="Join call" onClick={joinCall} disabled={joinVoice.isPending} className="flex size-10 cursor-pointer items-center justify-center rounded-full border-2 border-dashed border-primary text-primary focus-visible:ring-2 focus-visible:ring-ring disabled:cursor-not-allowed">{joinVoice.isPending ? <Loader2 className="size-4 animate-spin" /> : <Phone className="size-[19px]" />}</button>}</div></aside>;
}
