...
export function ActiveSessionWidget() {
  ...
  return <Link href={`/sessions/${sessionId}`} className="fixed bottom-10 right-10 z-40 w-[310px] rounded-[16px] border-[3px] border-border bg-card p-5 shadow-[6px_6px_0_rgba(0,0,0,0.3)] transition-transform hover:-translate-y-1"><span className="flex items-center gap-2"><Radio className="size-3 text-green" /><span className="font-bold text-[13px] text-card-foreground">Game in progress</span></span><span className="mt-3 block text-[13px] text-muted-foreground">{activePlayer?.displayName ? `${activePlayer.displayName} is placing a card` : "A round is in progress"}</span><span className="mt-2 flex items-center gap-2 text-[13px] font-display text-blue"><Clock3 className="size-[18px]" />Round {currentRound?.roundNumber ?? ""}</span><span className="mt-4 inline-flex items-center gap-2 rounded-full bg-primary px-4 py-2.5 font-display text-xs text-primary-foreground">Return to game <ArrowRight className="size-4" /></span></Link>;
}
