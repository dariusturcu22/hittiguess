...
const VISUALIZER_QUIET_BAR_HEIGHT_PERCENT = 20;
const VISUALIZER_LOUD_BAR_RANGE_PERCENT = 80;
...
export default function GameSessionPage({ params }: PageProps) {
  ...
  function skipBetting() {
    if (realtime.skipBetting()) {
      setFeedbackMessage("Betting skipped. Waiting for the reveal.");
    } else {
      setFeedbackMessage("The game connection is not ready. Try again in a moment.");
    }
  }
  ...
  return <main className="relative flex min-h-[720px] flex-col overflow-hidden px-6 py-8 sm:px-14 sm:py-9">
    ...
    <section className="relative flex min-h-[530px] flex-1 items-center justify-center"><p className="absolute bottom-[calc(50%+108px)] font-bold text-xs uppercase tracking-[0.24em] text-muted-foreground">Timeline</p><div ref={timelineRef} className={`w-full max-w-[1100px] overflow-x-auto px-10 py-4 transition-opacity ${isBettingPrep ? "opacity-70 saturate-50" : ""}`} style={{ maskImage: TIMELINE_MASK }}><div className="flex w-max items-center gap-[18px]"><DropTarget isVisible={(isDraggingCard && canPlaceCard) || (isSelectingBet && canBet)} position={0} onPlace={selectTimelinePosition} />{timeline.map((card, index) => <div key={`${card.songId}-${card.position}`} className={`flex items-center gap-[18px] transition-transform duration-500 ${currentRound?.status === "REVEALED" || currentRound?.status === "SCORED" ? "scale-105" : ""}`}><TimelineCard card={card} index={index} /><DropTarget isVisible={(isDraggingCard && canPlaceCard) || (isSelectingBet && canBet)} position={index + 1} onPlace={selectTimelinePosition} /></div>)}{timeline.length === 0 ? <p className="py-12 text-sm text-muted-foreground">Your timeline will appear when the round starts.</p> : null}</div></div><button type="button" aria-label="View earlier timeline cards" onClick={() => timelineRef.current?.scrollBy({ left: -360, behavior: "smooth" })} className="absolute left-0 top-1/2 flex size-10 -translate-y-1/2 items-center justify-center rounded-full border-2 border-border bg-card shadow-[3px_3px_0_rgba(0,0,0,0.25)]"><ChevronLeft className="size-5" /></button><button type="button" aria-label="View later timeline cards" onClick={() => timelineRef.current?.scrollBy({ left: 360, behavior: "smooth" })} className="absolute right-0 top-1/2 flex size-10 -translate-y-1/2 items-center justify-center rounded-full border-2 border-border bg-card shadow-[3px_3px_0_rgba(0,0,0,0.25)]"><ChevronRight className="size-5" /></button>
      {currentRound && isDj ? <div className="absolute bottom-[calc(50%+178px)] left-1/2 flex w-max max-w-[calc(100%-24px)] -translate-x-1/2 flex-col items-center gap-5 sm:flex-row sm:items-center sm:gap-7"><SoundwaveCard levels={audioLevels} /><div className="max-w-[340px] text-center sm:text-left">{linkOutQuery.data?.watchUrl ? <a href={youtubeLinkOutHref(linkOutQuery.data.watchUrl, navigator.userAgent)} target={YOUTUBE_LINK_OUT_TARGET} rel={YOUTUBE_LINK_OUT_REL} onClick={() => setIsLinkOutOpen(true)} className="inline-flex items-center gap-2 rounded-full bg-primary px-5 py-3 font-display text-[13px] text-primary-foreground shadow-[3px_3px_0_rgba(0,0,0,0.28)]">Open on YouTube to play<ExternalLink className="size-4" /></a> : <span className="inline-flex items-center gap-2 rounded-full bg-primary px-5 py-3 font-display text-[13px] text-primary-foreground opacity-55">Open on YouTube to play<ExternalLink className="size-4" /></span>}<p className="mt-2 text-xs leading-relaxed text-muted-foreground">Play it out loud. Everyone else finds out what it is when the card gets revealed.</p>{isLinkOutOpen ? <button type="button" onClick={requestDjTabAudioShare} disabled={isTabAudioShared} className="mt-2 inline-flex items-center gap-2 rounded-full border-2 border-warning bg-warning/10 px-3 py-1.5 text-[11px] font-semibold text-warning disabled:opacity-80"><Volume2 className="size-3.5" />{isTabAudioShared ? "Sharing YouTube audio with the group" : "Share YouTube audio with the group"}</button> : <div className="mt-2 inline-flex items-center gap-2 rounded-full border-2 border-warning bg-warning/10 px-3 py-1.5 text-[11px] font-semibold text-warning"><Volume2 className="size-3.5" />Shares your tab or system audio with the group.</div>}</div></div> : null}
      ...
      {canBet ? <div className="absolute bottom-[calc(50%+148px)] left-1/2 flex -translate-x-1/2 flex-col items-center gap-3"><span className="font-bold text-[11px] uppercase tracking-[0.2em] text-warning">Betting window</span><div className="flex flex-wrap justify-center gap-2"><button type="button" onClick={() => setIsSelectingBet((currentValue) => !currentValue)} className="rounded-full border-2 border-warning bg-warning/10 px-5 py-3 font-display text-xs text-warning shadow-[3px_3px_0_rgba(0,0,0,0.22)]">{isSelectingBet ? "Choose a timeline gap" : "Use 1 token to bet"}</button><button type="button" onClick={skipBetting} className="rounded-full border-2 border-border bg-card px-4 py-3 text-xs font-semibold text-card-foreground">Skip betting</button></div><span className="text-[11px] text-muted-foreground">Pick where you think the locked card belongs.</span></div> : null}
