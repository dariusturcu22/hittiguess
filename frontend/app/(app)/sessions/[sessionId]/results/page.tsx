...
function RankingRow({ player, index }: { player: PlayerResultDTO; index: number }) {
  return <div className="flex items-center gap-3 border-b border-border/70 py-3 last:border-0"><span className="w-5 font-display text-sm text-muted-foreground">{player.rank ?? index + 1}</span><span className={`avatar-initial flex size-8 items-center justify-center rounded-full font-display text-xs text-primary-foreground ${PLAYER_COLORS[index % PLAYER_COLORS.length]}`}>{initial(player.displayName)}</span><span className="flex-1 text-sm font-semibold text-card-foreground">{player.displayName ?? "Player"}</span><span className="font-display text-base text-card-foreground">{player.cardCount ?? 0}</span></div>;
}
